package android.support.p009v4.view.animation;
...
class PathInterpolatorApi14 implements Interpolator {
    ...
    /* renamed from: mX */
    private final float[] f1003mX;

    /* renamed from: mY */
    private final float[] f1004mY;
    ...
    PathInterpolatorApi14(Path path) {
        ...
        this.f1003mX = new float[i];
        this.f1004mY = new float[i];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < i; i2++) {
            pathMeasure.getPosTan((((float) i2) * length) / ((float) (i - 1)), fArr, null);
            this.f1003mX[i2] = fArr[0];
            this.f1004mY[i2] = fArr[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f1003mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f1003mX[i2]) {
                length = i2;
            } else {
                i = i2;
            }
        }
        float[] fArr = this.f1003mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f1004mY[i];
        }
        float[] fArr2 = this.f1004mY;
        float f3 = fArr2[i];
